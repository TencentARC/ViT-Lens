...
class PointNSATokenizer(nn.Module):
    def __init__(self, config):
        ...
        self.sa = PointNetSetAbstraction(
            npoint=config.num_group,
            radius=config.radius,
            nsample=config.group_size,
            in_channel=config.in_dim + 3,
            mlp=[64, 64, config.encoder_dims],
            group_all=False,
        )
        self.lift = nn.Sequential(
            nn.Conv1d(config.encoder_dims + 3, config.trans_dim, 1),
            rst.Lambda(lambda x: torch.permute(x, [0, 2, 1])),
            nn.LayerNorm([config.trans_dim]),
        )
