...
from training.data import get_data, get_audio_data
...
from training.zero_shot import test_audiotasks_core
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # test_metrics = test_audiotasks_core(test_loaders, model, tokenizer=get_tokenizer(args.model), args=args)
        tri_train_one_epoch(
            model,
            data,
            loss,
        ...
        if any(v in data for v in ("val", "imagenet-val", "imagenet-v2")):
            ...
            test_metrics = test_audiotasks_core(
                test_loaders, model, tokenizer=get_tokenizer(args.model), args=args
            )
        ...
        if args.save_logs:
            ...
            if args.save_best:
                ...
                if metric_ep > best_acc:
                    best_acc = metric_ep
                    prev_best_save_paths = glob.glob(
                        os.path.join(args.checkpoint_path, "checkpoint_best_*.pt")
                    )
                    for prev_pth in prev_best_save_paths:
                        os.remove(prev_pth)
                    best_save_path = os.path.join(
                        args.checkpoint_path, f"checkpoint_best_{completed_epoch}.pt"
                    )
