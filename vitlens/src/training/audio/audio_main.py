...
def main(args):
    ...
    if args.resume is not None:
        ...
        if "epoch" in checkpoint and not args.resume_ckpt_only:
        ...
        elif "epoch" in checkpoint and args.resume_ckpt_only:
            sd = checkpoint["state_dict"]
            if not args.distributed and next(iter(sd.items()))[0].startswith("module"):
                sd = {k[len("module.") :]: v for k, v in sd.items()}
            model.load_state_dict(sd)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # test_audiotasks_core(test_loaders, model, tokenizer=get_tokenizer(args.model), args=args)
        train_dual_one_epoch(
            model,
            data,
            loss,
            epoch,
