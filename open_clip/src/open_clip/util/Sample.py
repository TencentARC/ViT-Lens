...
def SampleCollator(cls, batch):
    sample_list = convert_batch_to_sample_list(batch)
    dta_type = getattr(cls, "vis_data_type", None)
    if dta_type is not None:
        sample_list.dataset_type = dta_type
    return sample_list
...
if __name__=="__main__":
    sample1 = Sample()
    sample1.img = torch.randn(1,224,224)
    sample1.imglist = [1,3,4]

    sample2 = Sample()
    sample2.img = torch.randn(1,224,224)
    sample2.imglist = [2,3,4]

    batch = convert_batch_to_sample_list([sample1, sample2, sample1, sample2])
    print(batch.imglist)

    print(isinstance(batch, dict))
    print(SampleList({"hello": torch.randn(2,2,3)}))