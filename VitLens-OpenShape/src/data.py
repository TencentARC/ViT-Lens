...
class Four(Dataset):
    def __init__(self, config, phase):
        ...
        self.text_embed_version = (
            "prompt_avg" if self.use_prompt_engineering else "original"
        )
        ...
        if self.use_text_filtering:
            self.gpt4_filtering = json.load(
                open(config.dataset.gpt4_filtering_path, "r")
            )
        logging.info("Phase %s: %d samples" % (phase, len(self.split)))

    def get_objaverse(self, meta):
        uid = meta["id"]
        data_path = meta["data_path"].replace(
            "/mnt/data/objaverse-processed", self.config.dataset.data_root
        )
        ...
        if "text" in self.text_source:
            if not (
                self.use_text_filtering and self.gpt4_filtering[uid]["flag"] == "N"
            ):
        ...
        if "retrieval_text" in self.text_source:
            if len(data["retrieval_text"]) > 0:
                ...
                )  # no prompt engineering for retrieval text
    ...
    def __getitem__(self, index: int):
        if self.use_knn_negative_sample == False:
            ...
            knn_idx = [0] + (
                np.random.choice(31, self.negative_sample_num - 1, replace=False) + 1
            ).tolist()
            for i in knn_idx:
...
def minkowski_collate_fn(list_data):
    ...
    return {
        ...
        "text_feat": [
            data["text_feat"] for data in list_data if data["text_feat"] is not None
        ],
        ...
        "has_text_idx": [
            i for i, data in enumerate(list_data) if data["text_feat"] is not None
        ],
    }
    ...
    if config.dataset.name == "Four":
        ...
        if phase == "train":
            ...
            sampler = DistributedSampler(
                dataset, num_replicas=world_size, rank=rank, drop_last=True
            )
        else:
            raise NotImplementedError("Phase %s not supported." % phase)
...
class ModelNet40Test(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        rgb = (
            rgb / 255.0
        )  # 100, scale to 0.4 to make it consistent with the training data
        if self.y_up:
            # swap y and z axis
            xyz[:, [1, 2]] = xyz[:, [2, 1]]
...
def minkowski_modelnet40_collate_fn(list_data):
    return {
        ...
        "category": torch.tensor(
            [data["category"] for data in list_data], dtype=torch.int32
        ),
    }
...
def make_modelnet40test(config):
    ...
    sampler = (
        DistributedSampler(
            dataset, num_replicas=config.world_size, rank=config.rank, shuffle=False
        )
...
class ObjaverseLVIS(Dataset):
    def __init__(self, config):
        ...
        self.clip_cat_feat = np.load(
            config.objaverse_lvis.clip_feat_path, allow_pickle=True
        )
    ...
    def __getitem__(self, index: int):
        data_path = self.split[index]["data_path"].replace(
            "/mnt/data/objaverse-processed", self.config.dataset.data_root
        )
        data = np.load(data_path, allow_pickle=True).item()
        n = data["xyz"].shape[0]
...
class ScanObjectNNTest(Dataset):
    def __init__(self, config):
        ...
        clip_feat = np.load(
            config.scanobjectnn.clip_feat_path, allow_pickle=True
        ).item()
        self.categories = [
            "bag",
            "bin",
    ...
    def __getitem__(self, index: int):
        ...
        if n != self.num_points:
            idx = np.random.choice(
                n, self.num_points
            )  # random.sample(range(n), self.num_points)
            xyz = xyz[idx]
            rgb = rgb[idx]
        if self.y_up:
            # swap y and z axis
