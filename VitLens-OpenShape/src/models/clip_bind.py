...
class CLIPBindWrap(nn.Module):
    def __init__(self, args):
        ...
        if hasattr(self.backbone, "proj"):
            if isinstance(self.backbone.proj, nn.Parameter):
                if self.backbone.proj.shape[-1] != args.model.out_channel:
                    ...
                    self.proj_layer = nn.Linear(
                        self.backbone.transformer.width, args.model.out_channel
                    )
            ...
            self.proj_layer = nn.Linear(
                self.backbone.eva_vit.embed_dim, args.model.out_channel
            )
    ...
    def lock(
        ...
        self.backbone.lock(
            unlocked_groups, freeze_bn_stats, unlock_cls, unlock_trans_first_n_layers
        )
        ...
        if self.args.skip_trans_first_n_layers is not None:
            if (
                self.args.skip_trans_first_n_layers > 0
            ):  # if skip some layers, unlock cls embed for training
                append_cls = True
